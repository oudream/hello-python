...
print((' '.join([str(item) for item in [x for x in range(2, 101) if not [x % i for i in range(2, x) if x % i == 0]]])))
print((' '.join([str(item) for item in [x for x in range(2, 101) if all([x % p != 0 for p in range(2, x)])]])))
...
# 一行代码实现快排算法
qsort = lambda arr: len(arr) > 1 and qsort(list([x for x in arr[1:] if x <= arr[0]])) + arr[0:1] + qsort(list([x for x in arr[1:] if x > arr[0]])) or arr


# 一行代码解决八皇后问题
[__import__('sys').stdout.write('\n'.join('.' * i + 'Q' + '.' * (8-i-1) for i in vec) + "\n========\n") for vec in __import__('itertools').permutations(list(range(8))) if 8 == len(set(vec[i]+i for i in range(8))) == len(set(vec[i]-i for i in range(8)))]
